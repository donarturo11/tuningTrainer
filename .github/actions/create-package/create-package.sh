...
create_package_linux() {
  ...
  sed -i "/^Keywords\(\[[a-zA-Z_.@]\+\]\)\?=/d;/^Version=/cVersion=1.0" \
    tuningTrainer/usr/share/applications/org.donarturo11.tuningTrainer.desktop
  ...
  LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:/usr/lib/x86_64-linux-gnu/pulseaudio" \
    ...
    tuningTrainer/usr/share/applications/org.donarturo11.tuningTrainer.desktop \
    -appimage
  mv tuningTrainer*.AppImage* "tuningTrainer-linux-$1-$(date +%F).AppImage"
...
create_package_macos() {
  ...
  popd >/dev/null
  echo "Create ZIP"
  bsdtar caf "tuningTrainer-mac-$1-$(date +%F).zip" tuningTrainer
  echo "::set-output name=package-name::tuningTrainer-mac-$1-$(date +%F).zip"
}
...
create_package_windows() {
  ...
  windeployqt tuningTrainer/tuningTrainer.exe
  echo "::endgroup::"
  #echo "Copy OpenSSL DLLs"
  local xbits="-x${platform#win}"
  #cp "C:\\Program Files\\OpenSSL\\lib"{ssl,crypto}"-1_1${xbits/-x32/}.dll" Pencil2D/
  echo "Create ZIP"
  "${WINDIR}\\System32\\tar" caf "tuningTrainer-${platform}-$1-$(date +%F).zip" tuningTrainer
  echo "::set-output name=package-name::tuningTrainer-${platform}-$1-$(date +%F).zip"
}
