...
set -eE
export $PACKAGE_NAME=tuningTrainer

create_package_linux() {
  echo "::group::Set up AppImage contents"
  make install INSTALL_ROOT="${PWD}/${PACKAGE_NAME}"
  echo "::endgroup::"
  ...
  export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:/usr/lib/x86_64-linux-gnu/pulseaudio" 
    ./linuxdeployqt-continuous-x86_64.AppImage \
    -executable=${PACKAGE_NAME} \
    -appimage
  mv ${PACKAGE_NAME}*.AppImage* "pencil2d-linux-$1-$(date +%F).AppImage"
  echo "::set-output name=package-name::${PACKAGE_NAME}-linux-$1-$(date +%F).AppImage"
  echo "::endgroup::"
}
...
create_package_macos() {
  ...
  python macdeployqtfix-master/macdeployqtfix.py \
    ${PACKAGE_NAME}.app/Contents/MacOS/Pencil2D \
  ...
  popd >/dev/null
  echo "Create ZIP"
  bsdtar caf "${PACKAGE_NAME}-mac-$1-$(date +%F).zip" Pencil2D
  echo "::set-output name=package-name::${PACKAGE_NAME}-mac-$1-$(date +%F).zip"
}
